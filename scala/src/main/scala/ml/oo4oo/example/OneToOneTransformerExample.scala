...
import ml.oo4oo.dag.PipelineDAGHelper.PipelineDAGExtension
import ml.oo4oo.dag.{PipelineDAG, PipelineDAGHelper}
...
object OneToOneTransformerExample {
  ...
  def buildOO4OOPipeline(): PipelineDAG = {
    ...
    val contentFeatures = PipelineDAGHelper
      .placeHolder("df2")
      .select("id", "score", "level")
      .toNode()

    val castOp = new Cast(new CastModel(BasicType.Int, BasicType.String)).setInputCol("id").setOutputCol("id_str")

    val joinedDF = deviceFeatures
      .join(contentFeatures, Seq("id"), "full_outer")
      .select("id", "phone_number", "date", "score", "features")
      .withMLeapOp(Array(castOp))
      .withColumn("new_col", "LitOp", Array.empty[String], Map("value" -> "new_col_value"))
      .toNode()

    PipelineDAG(joinedDF)
  }

  def dagSave(path: String): Unit = {
    println(s"dag save")
    val (df1, df2, mleapDF1, mleapDF2) = buildDataFrames()

    val inputs = Map("df1" -> df1, "df2" -> df2)
    val dag    = buildOO4OOPipeline()
    dag.fit(inputs, verbose = 1)

    val output = dag.transform[DataFrame](inputs, verbose = 1)
    output.show()
  ...
  def main(args: Array[String]): Unit = {
    buildOO4OOPipeline().describe()

    dagSave("/tmp/dag_by_scala")
    dagLoad("/tmp/dag_by_scala")
